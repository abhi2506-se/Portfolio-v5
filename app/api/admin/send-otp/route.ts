 ...
 * Generates a 6-digit OTP, emails it to the admin's configured email,
 * and stores a signed hash of it in an httpOnly cookie (stateless — works on Vercel).
 *
 * Body: { email: string }  ← the address to send to (must match saved admin email if one is set)
 */
...
async function sendOtpEmail(to: string, otp: string): Promise<void> {
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f172a; margin: 0; padding: 40px 20px;">
      <div style="max-width: 480px; margin: 0 auto; background: #1e293b; border-radius: 16px; border: 1px solid #334155; overflow: hidden;">
        <div style="background: linear-gradient(135deg, #2563eb, #06b6d4); padding: 32px; text-align: center;">
          <div style="font-size: 32px; font-weight: 800; color: white; letter-spacing: -1px;">🔐 Portfolio Admin</div>
        ...
        <div style="padding: 32px; text-align: center;">
          ...
          <div style="background: #0f172a; border: 2px solid #334155; border-radius: 12px; padding: 24px; display: inline-block; margin: 0 auto;">
          ...
          <p style="color: #64748b; font-size: 12px; margin: 16px 0 0; padding-top: 16px; border-top: 1px solid #1e293b;">
  ...
  await transporter.sendMail({
    ...
    to,
    subject: `🔐 Your OTP: ${otp} — Portfolio Admin`,
    html,
  })
}
  ...
  try {
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
      return NextResponse.json(
        { success: false, message: 'Email not configured. Add SMTP_USER and SMTP_PASS to your .env.local file.' },
        { status: 503 }
      )
    }
    ...
    // If admin has a saved email, enforce it matches
    // If admin has a saved email, enforce it matches
const admin = getAdminCredentials()
const savedEmail = admin.username

if (savedEmail && savedEmail !== email) {
  return NextResponse.json(
    { success: false, message: 'Email does not match the registered admin email.' },
    { status: 403 }
  )
}
    ...
    const payload = JSON.stringify({ otpHash, email, expiry })
    ...
    return NextResponse.json(
      { success: false, message: `Failed to send email: ${msg}` },
      { status: 500 }
    )
  }
}
