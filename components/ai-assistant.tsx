...
export function AIAssistant() {
  ...
  const [chatWindowTop, setChatWindowTop] = useState<number | null>(null)
  ...
  // ── Auto-scroll (scroll only inside chat window, never the whole page) ────
  useEffect(() => {
  ...
  return (
    <>
      {/* ── Floating button — fixed LEFT side ── */}
      <div className="fixed bottom-6 left-6 z-50 flex flex-col items-start gap-2">
        <AnimatePresence>
          {showBubble && !open && (
            <motion.div
              ...
              className="bg-foreground text-background text-xs font-medium px-3 py-2 rounded-2xl rounded-bl-sm shadow-lg max-w-[170px] text-center"
            >
        ...
        <motion.button
          ...
          onClick={() => {
            if (!open) {
              // Capture current scroll so chat opens in the visible viewport area
              const vh = window.innerHeight
              const chatH = Math.min(580, vh - 96)
              const top = Math.max(8, window.scrollY + (vh - chatH) / 2)
              setChatWindowTop(top)
            }
            setOpen(o => !o)
            setShowBubble(false)
          }}
      ...
      {/* ── Chat window — opens at current viewport position, left side ── */}
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={chatWindowTop !== null ? {
              position: 'absolute',
              top: chatWindowTop,
              left: '1.5rem',
              zIndex: 50,
            } : undefined}
            className={chatWindowTop === null
              ? 'fixed bottom-24 left-4 sm:left-6 z-50 w-[calc(100vw-2rem)] sm:w-[400px] max-h-[580px] bg-background border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col relative'
              : 'w-[calc(100vw-2rem)] sm:w-[400px] max-h-[580px] bg-background border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col'
            }
          >
            {/* ── Header ── */}
            <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-blue-600 to-cyan-500 flex-shrink-0">
              ...
              <div className="ml-auto flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-white/10 hover:bg-white/20 text-white/80 text-xs rounded-full transition-colors"
                >
            ...
            {/* ── Messages ── */}
            <div className="chat-messages-container flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
              {messages.map((m) => (
                <motion.div
                  key={m.id}
