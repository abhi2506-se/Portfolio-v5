...
type Mode = 'recruiter' | 'developer' | 'client'
...
export function ExperienceModeSelector() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(STORAGE_KEY) as Mode | null
    if (saved && MODES.find(m => m.id === saved)) { setSelected(saved); setDismissed(true) }
      ...
      <AnimatePresence>
        {selected && dismissed && (
          <motion.div
            ...
            exit={{ y: -48, opacity: 0 }}
            className={`fixed top-16 left-0 right-0 z-40 flex items-center justify-center gap-3 py-2 px-4 text-xs font-medium backdrop-blur-md border-b ${
              selected === 'recruiter' ? 'bg-blue-500/10 border-blue-500/20 text-blue-600' :
              selected === 'developer' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600' :
